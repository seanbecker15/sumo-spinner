...
class Game {
  ...
  tick() {
    ...
    if (this.fourplayers) {
      this.clientA.emit("update", {
        spinners: [this.spinnerA, this.spinnerB, this.spinnerC, this.spinnerD],
        powerup: this.powerup,
      });
      this.clientB.emit("update", {
        spinners: [this.spinnerB, this.spinnerA, this.spinnerC, this.spinnerD],
        powerup: this.powerup,
      });
      this.clientC.emit("update", {
        spinners: [this.spinnerC, this.spinnerA, this.spinnerB, this.spinnerD],
        powerup: this.powerup,
      });
      this.clientD.emit("update", {
        spinners: [this.spinnerD, this.spinnerA, this.spinnerB, this.spinnerC],
        powerup: this.powerup,
      });
    } else {
      this.clientA.emit("update", {
        spinners: [this.spinnerA, this.spinnerB],
        powerup: this.powerup,
      });
      this.clientB.emit("update", {
        spinners: [this.spinnerB, this.spinnerA],
        powerup: this.powerup,
      });
    }
  ...
  detectCollision() {
    if (this.distanceBetween() < this.spinnerA.radius + this.spinnerB.radius) {
      ...
      this.spinnerA.dx =
        (this.spinnerB.dx * this.spinnerB.size * this.spinnerB.dtheta) / 5;
      this.spinnerA.dy =
        (this.spinnerB.dy * this.spinnerB.size * this.spinnerB.dtheta) / 5;
      ...
      if (this.spinnerA.dtheta > 6) this.spinnerA.dtheta--;
      if (this.spinnerB.dtheta > 6) this.spinnerB.dtheta--;
    }
    if (this.fourplayers) {
      if (
        this.distanceBetween(this.spinnerA, this.spinnerC) <
        this.spinnerA.radius + this.spinnerC.radius
      ) {
        ...
        const tmpy = this.spinnerA.dy;
        this.spinnerA.dx =
          (this.spinnerC.dx * this.spinnerC.size * this.spinnerC.dtheta) / 5;
        this.spinnerA.dy =
          (this.spinnerC.dy * this.spinnerC.size * this.spinnerC.dtheta) / 5;
        this.spinnerC.dx =
          (tmpx * this.spinnerA.size * this.spinnerA.dtheta) / 5;
        this.spinnerC.dy =
          (tmpy * this.spinnerA.size * this.spinnerA.dtheta) / 5;
        if (this.spinnerA.dtheta > 6) this.spinnerA.dtheta--;
        if (this.spinnerC.dtheta > 6) this.spinnerC.dtheta--;
      }
      if (
        this.distanceBetween(this.spinnerA, this.spinnerD) <
        this.spinnerA.radius + this.spinnerD.radius
      ) {
        ...
        const tmpy = this.spinnerA.dy;
        this.spinnerA.dx =
          (this.spinnerD.dx * this.spinnerD.size * this.spinnerD.dtheta) / 5;
        this.spinnerA.dy =
          (this.spinnerD.dy * this.spinnerD.size * this.spinnerD.dtheta) / 5;
        this.spinnerD.dx =
          (tmpx * this.spinnerA.size * this.spinnerA.dtheta) / 5;
        this.spinnerD.dy =
          (tmpy * this.spinnerA.size * this.spinnerA.dtheta) / 5;
        if (this.spinnerA.dtheta > 6) this.spinnerA.dtheta--;
        if (this.spinnerD.dtheta > 6) this.spinnerD.dtheta--;
      }
      if (
        this.distanceBetween(this.spinnerB, this.spinnerC) <
        this.spinnerB.radius + this.spinnerC.radius
      ) {
        ...
        this.spinnerB.dy =
          (this.spinnerC.dy * this.spinnerC.size * this.spinnerC.dtheta) / 5;
        this.spinnerC.dx =
          (tmpx * this.spinnerB.size * this.spinnerB.dtheta) / 5;
        this.spinnerC.dy =
          (tmpy * this.spinnerB.size * this.spinnerB.dtheta) / 5;
        if (this.spinnerB.dtheta > 6) this.spinnerB.dtheta--;
        if (this.spinnerC.dtheta > 6) this.spinnerC.dtheta--;
      }
      if (
        this.distanceBetween(this.spinnerB, this.spinnerD) <
        this.spinnerB.radius + this.spinnerD.radius
      ) {
      ...
      if (
        this.distanceBetween(this.spinnerC, this.spinnerD) <
        this.spinnerC.radius + this.spinnerD.radius
      ) {
        ...
        this.spinnerC.dy =
          (this.spinnerD.dy * this.spinnerD.size * this.spinnerD.dtheta) / 5;
        this.spinnerD.dx =
          (tmpx * this.spinnerC.size * this.spinnerC.dtheta) / 5;
        this.spinnerD.dy =
          (tmpy * this.spinnerC.size * this.spinnerC.dtheta) / 5;
        if (this.spinnerC.dtheta > 6) this.spinnerC.dtheta--;
        if (this.spinnerD.dtheta > 6) this.spinnerD.dtheta--;
      }
    }
    if (
      this.powerup &&
      this.distanceBetween(this.spinnerA, this.powerup) < this.spinnerA.radius
    ) {
    ...
    if (
      this.powerup &&
      this.distanceBetween(this.spinnerB, this.powerup) < this.spinnerB.radius
    ) {
    ...
    if (this.fourplayers) {
      if (
        this.powerup &&
        this.distanceBetween(this.spinnerC, this.powerup) < this.spinnerC.radius
      ) {
      ...
      if (
        this.powerup &&
        this.distanceBetween(this.spinnerD, this.powerup) < this.spinnerD.radius
      ) {
...
io.on("connection", function (client) {
  ...
  client.on("waitForGame", function (name) {
    client.name = name;
    console.log(
      `Client ${client.id} with name ${client.name} has started waiting for a game`
    );
  ...
  client.on("waitForGameFour", function (name) {
    client.name = name;
    console.log(
      `Client ${client.id} with name ${client.name} has started waiting for a 4-player game`
    );
...
setInterval(function () {
  ...
  const playersOnline = Object.keys(clients)
    .map((k) => clients[k])
    .filter((el) => el.isConnected)
    .reduce((acc, _) => acc + 1, 0);
  for (var clientid in clients) {
    const client = clients[clientid];
    if (client.isConnected) client.emit("stats", { topfive, playersOnline });
  }
  ...
  if (clientsWaitingFour.length > 3 && gamesInProgress < maxGames) {
    let [clientIdA, clientIdB, clientIdC, clientIdD] =
      clientsWaitingFour.splice(0, 4);
